...
- 🔄 **多协议支持**：同时支持VLESS、Trojan、Shadowsocks三种协议
...
4. 配置你的代理设置：
   ...
   - **Password**：用于Trojan和Shadowsocks协议的密码
...
| **仅ProxyIP** | 严格使用ProxyIP，不回退 | 可自定义出口IP，灵活性高 | 需要特定地区IP，绕过地区限制 |
...
| 协议 | 支持的连接方式 | 特点 |
|------|----------------|------|
| **VLESS** | 仅直连、仅SOCKS5、仅ProxyIP、直连+SOCKS5、直连+ProxyIP、直连+SOCKS5+ProxyIP | 功能最全面，支持所有连接模式 |
| **Trojan** | 仅直连、仅ProxyIP | 配置简单，适合快速部署 |
| **Shadowsocks** | 仅直连、仅ProxyIP | 加密性强，不易被检测 |
